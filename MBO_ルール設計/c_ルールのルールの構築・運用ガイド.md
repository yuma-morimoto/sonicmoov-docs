...
- ルールの要不要は案件ごとに異なりますが、判断基準や制約が多い案件ほど効果が出やすくなります。
- ルールは、AI に案件の前提を毎回説明し直す手間を減らし、レビューで起きやすいズレを減らすためのものです。
- FEU では、個人の使い方に閉じず、チームで再現できる最低限の共通ルールを持つことに意味があります。

## 2. ルールを構築する

### まずは最小構成で始める

- 初回から細かく作り込まず、よく起きる手戻りを防ぐ内容だけを書きます。
- 目安は `100〜200行`、長くても `300行` 以内です。長くなる場合は、重複や細かすぎる指示を減らします。

### ルールテンプレート

- 構成は固定ではありません。案件に合わせて増減しつつ、ゼロから書き始めないための起点として使います。
...
- `実装ガイドライン`: AI が守るべき判断基準や実装上の制約を書きます。
- `ドメイン用語`: 案件固有の用語や、一般と意味がずれる言葉を定義します。
- `ディレクトリ構成`: 主要ディレクトリの役割を書き、編集場所の迷いを減らします。

### ルールを書くときのコツ

- 具体的なコード例は基本的に書かず、判断基準を中心に書きます。コード断片はすぐ古くなり、コンテキストも圧迫しやすいためです。
- コーディングスタイルは、レビューで繰り返し指摘されるものに絞って書きます。
- ドメイン知識や業務ルールは、汎用ルールより優先して残します。
- 初稿は AI に作らせても構いませんが、採用する内容は人間が取捨選択します。
...
- 新しく入るメンバーがすぐ試せるように、まずは `*.example` を配布し、各自がローカルで微調整する運用にします。

### 更新時の見直し方

- 更新するときも、「ルールを書くときのコツ」を見直し、情報を増やしすぎないように整理します。
...
- AI にルールの自己改善を任せきるのはまだ難しいため、草案の作成までに留めます。
- ルールは万能ではありません。案件理解やレビューの代わりではなく、あくまで安全に進めるための補助として使います。
